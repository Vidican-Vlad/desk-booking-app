...
const assignDesk = async (req, res) => {
	try {
		...
		if (user.desk == desk._id && desk.Owner == user._id)
			return res
				.status(400)
				.json({ msg: "desk was already assigned to this user" });
		user.desk = desk._id;
		desk.Owner = user._id;
...
const unassignDesk = async (req, res) => {
	try {
		...
		return res
			.status(200)
			.json({ msg: "assignemnet removed, desk is now free" });
	} catch (err) {
		console.log(err);
		return res.status(400).json(err);
	}
...
const bookDesk = async (req, res) => {
	try {
		...
		if (!Booking)
			res
				.status(400)
				.json({ msg: "There was a problem with booking this desk" });
...
const cancelBooking = async (req, res) => {
	try {
		...
		if (req.booking.Owner != req.auth._id && !req.auth.admin)
			return res
				.status(400)
				.json({ msg: "you do not have the permission to cancel this booking" });
...
function validateDesk(desk) {
	...
	if (
		...
		!desk.name ||
		typeof desk != "object" ||
		isStringInvalid(desk.name, 3)
	)
		return false;
	if (
		!validatePoint(desk.upLeft) ||
		!validatePoint(desk.upRight) ||
		!validatePoint(desk.downLeft) ||
		!validatePoint(desk.downRight)
	)
		return false;
	return true;
}
...
function validatePoint(point) {
	if (!point || typeof point != "object" || !point.x || !point.y) return false;
	return true;
}
